...
class TestShellCommands(Shell):
    """Tests migrations.py commands"""

    def test_help(self):
        """Displays default help dialog"""
        self.assertEqual(self.env.run('migrations -h').returncode, 0)
        self.assertEqual(self.env.run('migrations --help').returncode, 0)
        self.assertEqual(self.env.run('migrations help').returncode, 0)
    ...
    def test_help_commands(self):
        ...
        for cmd in api.__all__:
            result = self.env.run('migrations help %s' % cmd)
    ...
    def test_shutdown_logging(self):
        ...
        result = self.env.run('migrations create %s repository_name' % repos)
        result = self.env.run('migrations version %s --disable_logging' % repos)
        self.assertEqual(result.stdout, '')
        result = self.env.run('migrations version %s -q' % repos)
        self.assertEqual(result.stdout, '')
    ...
    def test_main_with_runpy(self):
        ...
        try:
            ...
            run_module('migrations.versioning.shell', run_name='__main__')
    ...
    def test_script(self):
        ...
        result = self.env.run('migrations script --repository=%s Desc' % repos)
        self.assertTrue(os.path.exists('%s/versions/001_Desc.py' % repos))

        result = self.env.run('migrations script More %s' % repos)
        self.assertTrue(os.path.exists('%s/versions/002_More.py' % repos))

        result = self.env.run('migrations script "Some Random name" %s' % repos)
    ...
    def test_script_sql(self):
        ...
        result = self.env.run('migrations script_sql mydb foo %s' % repos)
        ...
        result = self.env.run('migrations script_sql postgres foo --repository=%s' % repos)
    ...
    def test_manage(self):
        ...
        result = self.env.run('migrations manage %s --repository=/bla/' % script)
...
class TestShellRepository(Shell):
    ...
    def setUp(self):
        ...
        result = self.env.run('migrations create %s repository_name' % self.path_repos)

    def test_version(self):
        """Correctly detect repository version"""
        # Version: 0 (no scripts yet); successful execution
        result = self.env.run('migrations version --repository=%s' % self.path_repos)
        self.assertEqual(result.stdout.strip(), "0")

        # Also works as a positional param
        result = self.env.run('migrations version %s' % self.path_repos)
        self.assertEqual(result.stdout.strip(), "0")

        # Create a script and version should increment
        result = self.env.run('migrations script Desc %s' % self.path_repos)
        result = self.env.run('migrations version %s' % self.path_repos)
        self.assertEqual(result.stdout.strip(), "1")

    def test_source(self):
        """Correctly fetch a script's source"""
        result = self.env.run('migrations script Desc --repository=%s' % self.path_repos)
        ...
        result = self.env.run('migrations source 1 --repository=%s' % self.path_repos)
        self.assertEqual(result.stdout.strip(), source.strip())

        # We can also send the source to a file... test that too
        result = self.env.run('migrations source 1 %s --repository=%s' %
...
class TestShellDatabase(Shell, DB):
    ...
    def test_version_control(self):
        ...
        result = self.env.run('migrations drop_version_control %(url)s %(repos)s'\
            % locals(), expect_error=True)
        self.assertEqual(result.returncode, 1)
        result = self.env.run('migrations version_control %(url)s %(repos)s' % locals())

        # Clean up
        result = self.env.run('migrations drop_version_control %(url)s %(repos)s' % locals())
    ...
    def test_wrapped_kwargs(self):
        ...
        path_repos = repos = self.tmp_repos()
        url = self.url
        result = self.env.run('migrations create --name=repository_name %s' % repos)
        result = self.env.run('migrations drop_version_control %(url)s %(repos)s' % locals(), expect_error=True)
        self.assertEqual(result.returncode, 1)
        result = self.env.run('migrations version_control %(url)s %(repos)s' % locals())
    ...
    def test_version_control_specified(self):
        ...
        url = self.url
        result = self.env.run('migrations create --name=repository_name %s' % path_repos)
        result = self.env.run('migrations drop_version_control %(url)s %(path_repos)s' % locals(), expect_error=True)
        self.assertEqual(result.returncode, 1)
        ...
        for i in range(version):
            result = self.env.run('migrations script Desc --repository=%s' % path_repos)

        # Repository version is correct
        result = self.env.run('migrations version %s' % path_repos)
        self.assertEqual(result.stdout.strip(), str(version))

        # Apply versioning to DB
        result = self.env.run('migrations version_control %(url)s %(path_repos)s %(version)s' % locals())

        # Test db version number (should start at 2)
        result = self.env.run('migrations db_version %(url)s %(path_repos)s' % locals())
        self.assertEqual(result.stdout.strip(), str(version))

        # Clean up
        result = self.env.run('migrations drop_version_control %(url)s %(path_repos)s' % locals())
    ...
    def test_upgrade(self):
        ...
        repos_path = self.tmp()
        result = self.env.run('migrations create %(repos_path)s %(repos_name)s' % locals())
        self.assertEqual(self.run_version(repos_path), 0)

        # Version the DB
        result = self.env.run('migrations drop_version_control %s %s' % (self.url, repos_path), expect_error=True)
        result = self.env.run('migrations version_control %s %s' % (self.url, repos_path))

        # Upgrades with latest version == 0
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        result = self.env.run('migrations upgrade %s %s' % (self.url, repos_path))
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        result = self.env.run('migrations upgrade %s %s' % (self.url, repos_path))
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        result = self.env.run('migrations upgrade %s %s 1' % (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 1)
        result = self.env.run('migrations upgrade %s %s -1' % (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)

        # Add a script to the repository; upgrade the db
        result = self.env.run('migrations script Desc --repository=%s' % (repos_path))
        self.assertEqual(self.run_version(repos_path), 1)
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)

        # Test preview
        result = self.env.run('migrations upgrade %s %s 0 --preview_sql' % (self.url, repos_path))
        result = self.env.run('migrations upgrade %s %s 0 --preview_py' % (self.url, repos_path))

        result = self.env.run('migrations upgrade %s %s' % (self.url, repos_path))
        self.assertEqual(self.run_db_version(self.url, repos_path), 1)

        # Downgrade must have a valid version specified
        result = self.env.run('migrations downgrade %s %s' % (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)
        result = self.env.run('migrations downgrade %s %s -1' % (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)
        result = self.env.run('migrations downgrade %s %s 2' % (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)
        self.assertEqual(self.run_db_version(self.url, repos_path), 1)

        result = self.env.run('migrations downgrade %s %s 0' % (self.url, repos_path))
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        ...
        result = self.env.run('migrations drop_version_control %s %s' % (self.url, repos_path))
    ...
    def _run_test_sqlfile(self, upgrade_script, downgrade_script):
        ...
        result = self.env.run('migrations create %s %s' % (repos_path, repos_name))
        result = self.env.run('migrations drop_version_control %s %s' % (self.url, repos_path), expect_error=True)
        result = self.env.run('migrations version_control %s %s' % (self.url, repos_path))
        self.assertEqual(self.run_version(repos_path), 0)
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        ...
        result = self.env.run('migrations script_sql %s --repository=%s' % ('postgres', repos_path))
        self.assertEqual(self.run_version(repos_path), 1)
    ...
    def test_command_test(self):
        ...
        result = self.env.run('migrations create repository_name --repository=%s' % repos_path)
        result = self.env.run('migrations drop_version_control %s %s' % (self.url, repos_path), expect_error=True)
        result = self.env.run('migrations version_control %s %s' % (self.url, repos_path))
        self.assertEqual(self.run_version(repos_path), 0)
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        ...
        result = self.env.run('migrations script Desc %s' % repos_path)
        result = self.env.run('migrations test %s %s' % (self.url, repos_path))
        self.assertEqual(self.run_version(repos_path), 1)
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        ...
        result = self.env.run('migrations test %s %s bla' % (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)
        self.assertEqual(self.run_version(repos_path), 1)
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        ...
        from migrations.changeset import schema
        ...
        def upgrade():
            ...
            # named 'migrate_engine' imported from migrations.
            meta.create_all()
    ...
    def test_rundiffs_in_shell(self):
        ...
        repos_path = self.tmp()
        script_path = self.tmp_py()
        model_module = 'migrations.tests.fixture.models:meta_rundiffs'
        old_model_module = 'migrations.tests.fixture.models:meta_old_rundiffs'
        ...
        # Setup helper script.
        result = self.env.run('migrations manage %s --repository=%s --url=%s --model=%s'\
            % (script_path, repos_path, self.url, model_module))
        self.assertTrue(os.path.exists(script_path))

        # Model is defined but database is empty.
        result = self.env.run('migrations compare_model_to_db %s %s --model=%s' \
        ...
        result = self.env.run('migrations update_db_from_model %s %s %s'\
            % (self.url, repos_path, model_module))
        self.assertEqual(self.run_version(repos_path), 0)
        ...
        result = self.env.run('migrations compare_model_to_db %s %s %s'\
            % (self.url, repos_path, model_module))
        ...
        result = self.env.run('migrations create_model %s %s' % (self.url, repos_path))
        temp_dict = dict()
  ...
  ##Column('passwd', String(length=None, convert_unicode=False, assert_unicode=None))""" in result.stdout)
        ...
        #result = self.env.run('migrations make_update_script_for_model', expect_error=True)
        #self.assertTrue('Not enough arguments' in result.stderr)

        #result_script = self.env.run('migrations make_update_script_for_model %s %s %s %s'\
